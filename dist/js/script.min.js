const token="a77df976-cc73-4f1b-8224-7f0bab238df9",signIn=()=>{document.querySelector(".main");const a=document.querySelector(".header-block__btn"),s=document.querySelector(".modal");var e=document.querySelector(".close");const i=document.querySelector(".header-block__btn-create");a.addEventListener("click",()=>s.classList.toggle("modal--visible")),e.addEventListener("click",()=>s.classList.remove("modal--visible")),document.getElementById("login-button").onclick=async function(){const t=document.getElementById("username").value,r=document.getElementById("password").value;event.preventDefault(),await fetch("https://ajax.test-danit.com/api/v2/cards/login",{method:"POST",headers:{Authorization:"Bearer "+token,"Content-Type":"application/json"},body:JSON.stringify({email:t,password:r})}).then(e=>{e.ok?(s.style.display="none",a.style.display="none",i.style.display="flex",localStorage.setItem("username",t),localStorage.setItem("password",r),document.querySelector(".main__users-card").classList.remove("main__users-card--hidden")):console.log("Login failed")}).catch(e=>{console.error(e)})};var e=localStorage.getItem("username"),t=localStorage.getItem("password");e&&t&&(document.getElementById("username").value=e,document.getElementById("password").value=t,document.querySelector(".main__users-card").classList.remove("main__users-card--hidden"),document.getElementById("login-button").click())};signIn();class VisitModal{constructor(){this.modal=null}openModal(){var e=this.renderVisit(),t=document.createElement("div");t.innerHTML=e,this.modal=t.firstChild,document.body.appendChild(this.modal)}renderVisit(){return`<form class="main-modal__create-visit">
